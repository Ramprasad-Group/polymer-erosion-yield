...
Seven PSMILES entries, Kevlar, Nomex, PFA, Eymyd-F, PMDA-DAB, CN and ABS contained typos that were identified and corrected after the validation models in the paper were trained. Because the same PSMILES errors were present for both models, their comparison is not invalidated. Furthermore, omission of the affected rows from OME and Log-R2 calculations leave the metrics unchanged further suggesting these typos were inconsequential. To reproduce the reported validation results, run the pipeline with our `random/` and `rg/` files, while to rerun the pipeline using the corrected dataset, delete both `random/` and `rg/` before running.
