...
Core LLM pipeline (`prep` / `train` / `retrieve` / `infer` / `figures` / `autotune`):

```
pip install -r requirements.txt
```

For the traditional-ML benchmark (`ml-bench`), install the extra dependencies too:

```
pip install -r requirements-ml.txt
```

The gradient-boosting backends (xgboost / lightgbm / catboost) are skipped automatically at runtime if not installed. The polyBERT featurizer downloads the public checkpoint (`kuelumbus/polyBERT`) on first use.

## OpenAI API setup

Modes that submit fine-tune jobs or run inference need an OpenAI key, resolved in this order:

1. `openai_api_key.txt` in the working directory (one line, gitignored), or
