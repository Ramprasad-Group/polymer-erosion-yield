#!/usr/bin/env python3
"""
Unified LEO erosion-yield pipeline for fine-tuned GPT-4o and GPR using the same
cross-validation split strategies.

MODES
  LLM
    temp-tune      Sweep TEMP_GRID on the 5 restricted-group folds and write
    ...
    llm-ablation   Run the baseline, +layers, and +thickness LLM ablations on
  ...
  Reporting / figures
    plot-only      Redraw parity plots from a saved predictions CSV.
    validation-fig Build the 2 x 2 LLM/GPR validation figure for the
                   restricted-group and random splits.
    data-fig       Plot dataset sample counts, AO fluence, and erosion yield.
    descriptor-fig Plot pairwise numerical-descriptor relationships and erosion
                   yield relationships.
    tuning-fig     Build the epoch/temperature tuning figure from saved tuning
                   artifacts.
    gpr-ablation-fig
...
those split files are used. If both are absent, the corresponding splits are
rebuilt from the corrected canonical dataset using frozen memberships.
"""
...
NUM_COLS      = ["mission_time (yr)", "solar (esh)", "fluence"]
...
PARITY_YLAB_IN      = 1.15
PARITY_TICK_IN      = 0.50
PARITY_TITLE_IN     = 0.70
PARITY_XLAB_IN      = 0.80
PARITY_LABEL_PAD_IN = 0.35
...
def _rg_split_hash(rg_dir):
    """Hash all restricted-group train/test split files."""
    try:
        h = hashlib.md5()
...
def _gpr_cache_valid(run_dir, strategy, data_csv, out_dir, bits, radius, kernel,
                     rg_dir=None):
    """Check whether saved GPR results match the current data, split, and configuration."""
...
def _load_split_membership(pool, out_dir, strategy):
    """Rebuild train/test folds from frozen master-row test memberships."""
...
def generate_rg_splits(pool, n=5):
    """Rebuild the restricted-group folds from singleton chemistries in master-row order using SEED."""
...
def _paper_split_dirs(rg_dir):
    """Require the historical restricted-group and random split directories to be present or absent together."""
...
def metric_row(y_true, y_pred, **extra):
    """Return metric_ci() results and bootstrap metadata as one output row."""
...
MAX_ACTIVE_JOBS = 3
POLL_SECS     = 60
MAX_TRANSIENT_TRIES = 8
MAX_RATELIMIT_TRIES = 20
TRANSIENT_BACKOFF_CAP = 600
RATELIMIT_BACKOFF_CAP = 4 * 3600
...
def orchestrate(run_dir, units_epochs, wait=True):
    """Submit and poll fine-tuning units with retry handling and an active-job cap."""
    client = _client()
...
def infer_repeats(client, model, test_df, temperature, variant="base", repeats=INFER_REPEATS,
                  label="", raw_dir=None):
    """Run repeated inference per test row, return mean/sample-SD results, and optionally save raw repeats."""
...
def _cv_rg_epochs(args):
    """Read the epoch count used by the saved llm-cv restricted-group models."""
    m = _load_manifest(os.path.join(args.out_dir, "llm_cv"))
    eps = set()
    for k in range(5):
...
def mode_llm_temp_tune(args):
    """Sweep TEMP_GRID over the restricted-group folds using the saved llm-cv models."""
...
def mode_llm_epoch_tune(args):
    """Sweep EPOCH_GRID over the restricted-group folds, reusing a compatible llm-cv arm when available."""
    pool = load_master_csv(args.data_csv)
    rg = get_splits("restricted-group", pool, args.rg_dir, args.out_dir)
...
def mode_llm_cv(args):
    """Run LLM cross-validation while preserving existing strategy-level prediction artifacts."""
    pool = load_master_csv(args.data_csv)
...
def _cached_infer(client, run_dir, u, model, te, temp, variant="base", artifact_dir=None):
    """Return cached or newly inferred predictions for one split."""
...
def mode_llm_report(args):
    """Report completed llm-cv results from saved artifacts."""
    pool = load_master_csv(args.data_csv)
    temp = _best_temp(args)
    run_dir = os.path.join(args.out_dir, "llm_cv")
    m = _load_manifest(run_dir)
...
def mode_gpr_cv(args):
    """Run the selected GPR configuration across all cross-validation strategies."""
    pool = load_master_csv(args.data_csv)
...
def mode_gpr_prod(args):
    """Fit the production GPR and run 10-fold generalization cross-validation."""
    pool = load_master_csv(args.data_csv)
    coat = coating_levels(pool)
    bits, radius, kernel = _gpr_best(args)
...
def mode_gpr_ablation(args):
    """Run matched-control GPR ablations for baseline, layers, and thickness descriptors."""
    pool = load_master_csv(args.data_csv)
    coat = coating_levels(pool)
    bits, radius, kernel = _gpr_best(args)
    both = (pd.to_numeric(pool["layers"], errors="coerce").notna()
...
def _savefig_multi(fig, svg_path):
    """Save a figure in SVG, PDF, and EPS formats."""
...
def _equalize_parity_axes(fig, axes, suptitle_in, extra_bot_in=0.0):
    """Place ablation parity axes as equal-sized squares after measuring their decorations."""
    from matplotlib.transforms import Bbox
    fig.tight_layout()
    fig.canvas.draw()
    r = fig.canvas.get_renderer()
...
def finalize_parity_panels(fig):
    """Place parity-panel legends and metric boxes after final layout."""
    r = fig.canvas.get_renderer()
    for ax in fig.axes:
...
def _parity(y, mu, band, title, path, note, orient=None, flu=None, style=None):
    """Draw a single production-model parity panel."""
    st = PLOT_STYLE if style is None else style
    fig, ax = plt.subplots(figsize=(5.5, 5))
...
def mode_plot_only(args):
    """Redraw parity plots from a saved cross-validation predictions CSV."""
...
def mode_data_fig(args):
    """Build the dataset-description figure from the canonical CSV."""
...
def mode_descriptor_fig(args):
    """Build pairwise numerical-descriptor and erosion-yield scatterplots from the canonical CSV."""
...
def _ablation_plot_metadata(question):
    """Recover orientation and fluence from the saved ablation inference question."""
    q = str(question)
...
def mode_ablation_fig(args):
    """Build the LLM baseline/layers/thickness ablation figure from saved raw repeats."""
    run_dir = os.path.join(args.out_dir, "llm_ablation")
...
def mode_tuning_fig(args):
    """Build the epoch and temperature tuning figure from saved tuning artifacts."""
...
def mode_gpr_ablation_fig(args):
    """Build the GPR baseline/layers/thickness ablation figure from saved prediction files."""
